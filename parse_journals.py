...
def is_new_journal(url):
    """Checks whether journal is new (UI)"""
    header = {
    ...
    if re.findall('Find out more', content):    # the criterion of old/new journal. unique to new journals
        return True
...
def main():
    ...
    if new_journals:
        new_journal_parser = subprocess.run(['python', 'new_journal_parser2.py'], input=new_journals, encoding='utf-8')
